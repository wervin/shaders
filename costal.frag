...
layout (binding = 0) uniform UniformBufferoutFragColorbject
{
    vec3 resolution;
    float time;
}
...
void main()
{
    ...
    vec2 r = ubo.resolution.xy, uv = (fragCoord + fragCoord - r) / r.y, sun_pos = vec2(r.x / r.y * .42, -.53) //sun position 
    ...
    sh = rotate2D(sun_pos, noise(uv + ubo.time * .25) * .3); //big noise on the sky
    ...
    if (uv.y > -.4) //drawing the sky
    {
        ...
        h = (hash12(floor(id.xx)) * .5 + .25) * (ubo.time + 10.) * .25; //ring shift
        ...
        h = noise(t * vec2(.5, 1) - vec2(ubo.time * .2, 0)) //clouds
    ...
    if (uv.y < -.35) //drawing water
    {
        ...
        float cld = noise(-sh * vec2(.5, 1) - vec2(ubo.time * .2, 0)); //cloud density opposite the center of the sun
        ...
        for (float i = 1.; i > -1.; i--) //drawing a wave and its neighbors from above and below
        {
            ...
            {
                lc = fract(u) - .5;
                lc.y = (lc.y + (sin(uv.x * 12. - ubo.time * 3. + id.y + i)) * .25 - i) * 4.; //set the waveform and divide it into four strips
                ...
                lc.x += sin(ubo.time * (.5 + h * 2.)) * .5; //add a cyclic shift of the strips horizontally
    ...
    outFragColor = vec4(f, 1);

    ////////////////////// drawing the grass
    a = 0.;
    u = uv + noise(uv * 2.) * .1 + vec2(0, sin(uv.x * 1. + 3.) * .4 + .8);

    f = mix(vec3(.7, .6, .2), vec3(0, 1, 0), sin(ubo.time * .2) * .5 + .5); //color of the grass, changing from green to yellow and back again
    outFragColor = mix(outFragColor, vec4(f * .4, 1), step(u.y, .0)); //draw grass background
    ...
    if (u.y < 1.2)
    {
        for (float y = 0.; y > -3.; y--)
        {
            for (float x = -2.; x < 3.; x++)
            {
                ...
                lc -= vec2(.3, .5 - h * .4);
                lc.x += sin(((ubo.time * 1.7 + h * 2. - id.x * .05 - id.y * .05) * 1.1 + id.y * .5) * 2.) * (lc.y + .5) * .5;
    ...
    if (abs(uv.x + tree_pos.x - .1 - T * .1) < .6) // drawing the tree
    {
        ...
        for (float xs = 0.; xs < 4.; xs++) //drawing four layers of foliage
        {
            ...
            u += noise(u * vec2(2, 1) + vec2(-ubo.time + xs * .05, 0)) * vec2(-.25, .1) * smoothstep(.5, -1., u.y + .7) * .75; //leaves rippling in the wind
            ...
            * st(abs(lc.x - .5), .35, sm * xd * .5);
            ...
            f = hue((h + (sin(ubo.time * .2) * .5 + .5)) * .2).rgb - t.x; //color of the segment changes cyclically

            C = mix(C, vec4(mix(f * .15, f * .6 * (.7 + xs * .2), //mix outline and foreground color
            st(abs(lc.y - .5), .47, sm * yd) * st(abs(lc.x - .5), .2, sm * xd)), m), m);
        }

        outFragColor = mix(outFragColor, C, C.a * (1. - a));
    }
